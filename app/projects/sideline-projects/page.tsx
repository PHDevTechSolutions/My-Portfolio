...
const defaultProjects: MobileCardProps[] = [
    {
        title: "Ecodesk Mobile",
        description:
            "Mobile application for managing customer service requests and tickets on-the-go. Features real-time notifications, ticket tracking, and offline mode for field technicians.",
        tags: ["React Native", "Expo", "Supabase", "Push Notifications", "Offline Sync"],
        image: "/mobile/ecodesk-mobile.png",
    ...
    },
    {
        ...
        description:
            "Mobile CRM companion for Taskflow platform. Manage sales activities, view daily reports, track attendance, and access customer data from anywhere.",
        tags: ["Flutter", "Dart", "Firebase", "REST API", "Biometric Auth"],
        image: "/mobile/taskflow-mobile.png",
        develop: "Leroux Y Xchire",
        assign: "Sales Team",
        links: {
            demo: "#",
    ...
    },
    {
        ...
        description:
            "Barcode scanning application for warehouse inventory management. Features real-time sync with SAP, batch scanning, and stock level alerts.",
        tags: ["Kotlin", "Android", "ZXing", "SAP Integration", "Real-time Sync"],
        image: "/mobile/inventory-scanner.png",
        develop: "Leroux Y Xchire",
...
function MobileCard({ id, title, description, tags, image, links, develop, assign, className }: MobileCardProps) {
    return (
        <motion.div variants={cardVariants} className={cn("w-full max-w-[400px]", className)}>
            <Card className="group relative h-full overflow-hidden rounded-none border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-300 hover:border-white/30 hover:bg-white/10">
                ...
                <div className="p-5">
                    ...
                    {/* View Info Button */}
                    <Link href={id ? `/projects/sideline-projects/${id}` : '#'} className="block mt-4">
...
export default function ProjectPage() {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                setLoading(true);
                const data = await getProjectsByType("mobile");
    ...
    return (
        <>
            <nav className="w-full bg-black/80 backdrop-blur-md border-b border-white/10 sticky top-0 z-50">
                <div className="mx-auto max-w-7xl px-6 py-4 flex justify-between items-center">
                    ...
                    <div className="flex space-x-2 items-center">
                        ...
                        <Link
                            href="/projects/sideline-projects"
            ...
            <main
                ...
                role="main"
                aria-label="Mobile Application Projects"
            >
                {/* Loading State */}
                {loading && (
                ...
                {!loading && (
                    ...
                    >
                        {projects.map((project, idx) => (
                            <MobileCard key={project.id || idx} {...project} />
                        ))}
                    </motion.div>
                )}
